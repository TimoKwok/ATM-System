...
public class ATM {
    ...
    private ArrayList<User> userList = new ArrayList<User>(); //list of all users
    private ArrayList<Transaction> transactionList = new ArrayList<Transaction>(); //list of all transactions
    private static int[] systemBills = {200, 200, 200}; //5, 20, 50 FINAL
    ...
    public void viewBalance(User thisUser, int account){ //done
        if (account == 1){
            ...
            System.out.print("\n\n\nSavings Account Balance: $" + f.format(thisUser.getSavingsAccount().getBalance()));//view savings account belonging to user
        } else if (account == 3){
            System.out.print("\n\nChequing Account Balance: $" + f.format(thisUser.getChequingAccount().getBalance()));//view both
            System.out.print("\nSavings Account Balance: $" + f.format(thisUser.getSavingsAccount().getBalance()));      
        }
        delay(1);
    }
    ...
    public void withdraw(User thisUser, int account, double amount) throws InsufficientFundsException, MachineOutOfBillsException {
        ...
        try{
            ...
            if (account == 1){
                ...
                Transaction t = new Transaction(generateTransactionId(), dtf.format(LocalDateTime.now()), amount, thisUser.getCardNumber(), "CHEQUING", "TRANSFER", null, getBillRep(billCombos.get(choice - 1)), valid);
                transactionList.add(t);
                thisUser.addTransaction(t);
            } else if (account == 2){
                thisUser.getSavingsAccount().withdraw(amount);
                Transaction t = new Transaction(generateTransactionId(), dtf.format(LocalDateTime.now()), amount, thisUser.getCardNumber(), "SAVINGS", "TRANSFER", null, getBillRep(billCombos.get(choice - 1)), valid);
                transactionList.add(t);
                thisUser.addTransaction(t);
            }
    ...
    public static ArrayList<int[]> getBills(int amount) {
